...
| `/kitchen` | **A cookbook since 2026-09-05.** Dishes he chose, each with the publisher's recipe link, a shopping list with store links, and his notes; a box to ask for the next one from his phone. Read `KitchenOS/README.md`. Neon: `dish`, `inbox`, `cook_log`, `shop_tick`, `shop_extra` | yes (`/kitchen/api/note`, `/kitchen/api/inbox`, `/kitchen/api/shop`) |
| `/kitchen/shop` | **ONE SHOPPING LIST ACROSS EVERY DISH, since 2026-09-12**, in his words: "I want one big shopping list that knows everything across every recipe." The union of every `dish.list`, each product once however many dishes want it, split into buy / optional / already have, filterable by dish, tickable, with a box for things no recipe knows about. Deduplicates on the WALMART URL and not the name: one dish carries "Shredded mozzarella (Great Value Pizza Mozzarella)" and "More mozzarella" as two rows with one URL. The arithmetic is `src/lib/kitchen/shoplist.ts`, which imports no database so `shoplist.test.ts` (57 cases, in `verify.mjs`) can run it. **A `dish.list` item now carries `need`: buy, optional or owned.** That state used to live in prose inside the note and the price ("ESSENTIAL", "DO NOT BUY", `price: 'ALREADY OWNED'`), six spellings of three states, and a page that greps for them puts a thing he owns on a buy list the first time a session writes the idea a seventh way. An item with no valid `need` is not guessed at in either direction: it lands in an unsorted section the page counts at the top | yes (`/kitchen/api/shop`) |
