 ...
 *  Run: node content/swim/apply-schema.mjs
 *
 ...
 *
 *  Reads .env.local itself, like the curio applier it was copied from. Sourcing that file from a shell on Windows
...
const res = await client.query(
  `select table_name from information_schema.tables
    where table_schema='public' and table_name like 'swim_%' order by table_name`,
);
console.log('swim tables now present:', res.rows.map((r) => r.table_name));
await client.end();
