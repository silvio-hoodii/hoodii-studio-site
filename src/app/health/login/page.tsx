...
// one login for the whole hub, not a second password to remember. See src/lib/login-server.ts.
async function signIn(formData: FormData) {
  'use server';
  const to = String(formData.get('to') ?? '/health');
  const outcome = await signInWithPassword(String(formData.get('pw') ?? ''));
  if (outcome === 'ok') redirect(to.startsWith('/health') ? to : '/health');
  redirect(`/health/login?${outcome === 'not-configured' ? 'unconfigured' : 'bad'}=1&to=${encodeURIComponent(to)}`);
}
